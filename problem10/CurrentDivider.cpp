...
// A helper function similar tp VoltageDivider's update_network
void CurrentDivider::update_network()
{
...
CurrentDivider::CurrentDivider(double c, double res[], int num_res) : Network(2, num_res + 1), num_res(num_res)
{
...
void CurrentDivider::set_source_current(double c)
{
  ...
  // Updating the nodes by using the helper function
  update_network();
}
